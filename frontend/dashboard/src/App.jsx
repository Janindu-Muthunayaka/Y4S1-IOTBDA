...
function Sidebar() {
  ...
  // Hide sidebar on Driver Dashboard since it's a dedicated mobile view
...
function MainLayout() {
   const location = useLocation();
   // The mockup for QA Inspector matches its own layout, but driver is specifically full-screen mobile
   ...
   return (
    <div style={{ display: 'flex', minHeight: '100vh', width: '100%', justifyContent: isDriver ? 'center' : 'flex-start' }}>
      ...
      <div style={{ flex: 1, overflowX: 'auto', display: 'flex', flexDirection: 'column' }}>
        {!isQa && !isDriver && <h1 className="header-title" style={{ marginTop: '2rem' }}>Cold-Chain Logistics Nexus</h1>}
        <div className={!isQa && !isDriver ? "dashboard-container" : ""} style={!isQa && !isDriver ? { paddingTop: 0 } : { flex: 1 }}>
          <Routes>
